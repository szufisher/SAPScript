    ...
    for idx, order_line in enumerate(order_lines):
        ...
        if "Error in goods movements" in status_text:
            ...
            if "eriod" in error:    # Period 012/2019 is not open for account type S
                send_email('skipped due to period end closing', "co11n@xxx.com")
                break
...
def job():
    try:
        ...
        if session:            
            ...
            close_db(conn)            
            close_sap(session)            
            send_email('CO11N processed %s records' % len(order_lines), "co11n@xxx.com")
        else:
            send_email("Failed logon SAP", 'co11n@xxx.com')
